...
def fast_group_scad(double[::1, :] _X, double[::1] y, int n_samples, int n_features, int[::1] groups, int[::1] group_sizes, int[::1, :] group_to_feat, double[::1] group_scalars, int n_groups, double alpha, double gamma, int n_lambdas=100, double factor=0.01, int max_iter=MAX_ITER, double rtol=1e-5):
    ...
    cdef:
        ...
        double delta = 1.0e-5
        int inc = 1
        int working_set_flag = 0
        int i, g, _g, f, _f, n_iter, c_iter, tmp_iter, margin_iter, inital_iter
        ...
        double[::1] beta_regression = np.zeros(n_features, dtype = np.float64, order='F')
        ...
        int[::1] working_set = np.zeros(n_groups, dtype = np.int32, order='F')
    ...
    with nogil:
        ...
        tmp_iter = 0 # iteration stopped for a working set because python iterator cannot reset

        # hyperparameter m in our paper
        #initial_iter = 500
        initial_iter = 1000
        #initial_iter = 2000

        margin_iter = 1 # more than 1
        for l in range(n_lambdas):
            ...
            for w in range(n_groups): working_set[w] = 0
            ...
            for n_iter in range(max_iter):
                ...
                # working set
                if n_iter != 1 and ((n_iter - tmp_iter) == margin_iter or n_iter == initial_iter):
                    for g in range(n_groups):
                        ...
                        if working_set_flag == 0: # non-shrink subset
                            if n_iter == initial_iter:
                                # compute lower bound
                                norm_tmp -= (-K_norms[g, g]*beta_diff_norms[g] + ddot(&n_groups, &beta_diff_norms[0], &inc, &K_norms[0, g], &inc))/sqrt(n_samples)
                                # select working set
                                if norm_tmp > gamma * lamb * group_scalar:
                                    working_set[g] = 1
                                    cnt1+=1
                        elif working_set_flag == 1: # small shrink subset
                            if working_set[g] == 0:
                                # compute lower bound
                                norm_tmp -= (- K_norms[g, g]*beta_diff_norms[g] + ddot(&n_groups, &beta_diff_norms[0], &inc, &K_norms[0, g], &inc))/sqrt(n_samples) 
                                if norm_tmp > 2.0 * lamb * group_scalar:
                                    # compute upper bound
                                    norm_tmp += 2.0 * (- K_norms[g, g]*beta_diff_norms[g] + ddot(&n_groups, &beta_diff_norms[0], &inc, &K_norms[0, g], &inc))/sqrt(n_samples) 
                                    if norm_tmp <= gamma * lamb * group_scalar:
                                        working_set[g] = 1
                                        cnt2+=1
                        elif working_set_flag == 2: # large shrink subset
                            if working_set[g] == 0:
                                # compute lower bound
                                norm_tmp -= (- K_norms[g, g]*beta_diff_norms[g] + ddot(&n_groups, &beta_diff_norms[0], &inc, &K_norms[0, g], &inc))/sqrt(n_samples)
                                if norm_tmp > lamb * group_scalar:
                                    # compute upper bound
                                    norm_tmp += 2.0 * (- K_norms[g, g]*beta_diff_norms[g] + ddot(&n_groups, &beta_diff_norms[0], &inc, &K_norms[0, g], &inc))/sqrt(n_samples) 
                                    if sqrt(norm_tmp) <= 2.0 * lamb * group_scalar:
                                        working_set[g] = 1
                                        cnt3+=1
                        elif working_set_flag == 3: # full set
                            if working_set[g] == 0:
                                working_set[g] = 1
                                cnt4+=1
                # end of selecting working set

                # group scad on working set
                dcopy(&n_features, &beta[0], &inc, &beta_old[0], &inc)
                for g in range(n_groups):
                    if n_iter > initial_iter and working_set[g] == 0:
                        continue
                    group_scalar = group_scalars[g]

                    for f in range(group_to_feat[g][0], group_to_feat[g][0]+group_sizes[g]):
                        XTR[f] = Xy[f] # XTR = Xy instead of (1). test may be needed
                    ...
                    for f in range(group_to_feat[g][0], group_to_feat[g][0]+group_sizes[g]):
                        ...
                        XTR[f] /= n_samples 
                        XTR_ref[f] =  XTR[f] # reference vector
                    ...
                    double_tmp = 0.
                    for f in range(group_to_feat[g][0], group_to_feat[g][0]+group_sizes[g]):
                        double_tmp += (beta_old[f] - beta[f])**2
                ...
                if norm_tmp / double_tmp < rtol:
                    with gil:
                        print("STOP")
                        print(working_set_flag)
                        print(n_iter)
                    tmp_iter = n_iter
                    working_set_flag += 1
                    if working_set_flag > 3:
                        break
            with gil:
                print(cnt1)
                print(cnt2)
