...
def group_scad(double[::1, :] _X, double[::1] y, int n_samples, int n_features, int[::1] groups, int[::1] group_sizes, int[::1, :] group_to_feat, double[::1] group_scalars, int n_groups, double alpha, double gamma, int n_lambdas=100, double factor=0.01, int max_iter=MAX_ITER, double rtol=1e-5):
    ...
    with nogil:
        ...
        for g in range(n_groups):
            ...
            # eigen decomposition for standardization
            with gil:
        ...
        for l in range(n_lambdas):
            ...
            for n_iter in range(max_iter):
                dcopy(&n_features, &beta[0], &inc, &beta_old[0], &inc) # for checking convergence
                ...
                for g in range(n_groups):
                    ...
                    for f in range(group_to_feat[g][0], group_to_feat[g][0]+group_sizes[g]):
                        ...
                        XTR[f] /= n_samples # due to the objective
